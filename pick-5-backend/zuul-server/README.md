...
```bash
curl -i -H "Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJzaHVhaWNqIiwiYXV0aG9yaXRpZXMiOlsiUk9MRV9VU0VSIl0sImlhdCI6MTU0NjE5NTAxMiwiZXhwIjoxNTQ2MjgxNDEyfQ.WURA7reuNI-e-DkTOP4GIfV8vN-DAJ-kZRh6FUCHcfE" http://localhost:8080/backend/user
```
or without token:
```bash
curl -i http://localhost:8080/backend/guest/
```
